...
.dropdown-submenu:hover > .dropdown-menu {
    ...
    <!-- Tela Desabilitar Livro -->
    <div class="container form-container">
        <h1 class="text-center-custom">Desabilitar Livro</h1><br>

        <!-- Formulário para desabilitar livro -->
        <form class="form-control form-control-sm" method="POST" style="width: 50%; margin: 0 auto;">
            <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Digite o código do livro</label>
                <input type="text" class="form-control" id="exampleFormControlInput1" name="idLivros" required>
            </div>
            <select name="campos" class="form-select" aria-label="Escolha um campo para desabilitar">
                <option selected>Escolha um campo para desabilitar</option>
                <option value="situacaoLivro">Situação do livro</option>
            </select><br>
            <select id="novoDado" name="novoDado" class="form-select" aria-label="Escolha Ativo ou Inativo">
        ...
        if (isset($_POST['idLivros']) && $_POST['idLivros'] != "") {
            $desabilitar->desabilitarLivros($conexao, $_POST['campos'], $_POST['novoDado'], $_POST['idLivros']);
            echo '<div class="alert alert-success mt-3">Livro desabilitado com sucesso!</div>';
        }
        ?>
    </div>
